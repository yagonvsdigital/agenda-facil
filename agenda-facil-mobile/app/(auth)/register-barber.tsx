...
export default function RegisterBarber() {
  ...
  async function handleRegister() {
    ...
    setLoading(true)
    try {
      await api.auth.register({ name: name.trim(), phone: phone.trim(), role: 'barber' })
  ...
  async function handleSendOtp() {
    ...
    setLoading(true)
    try {
      await api.auth.register({ name: name.trim(), phone: phone.trim(), role: 'barber' }).catch(() => {})
      const res = await api.auth.sendOtp(phone.trim())
      setDevCode(res.dev_code ?? '')
      setStep('otp')
    } catch (e: any) { toast('error', e.message) }
    ...
    <SafeAreaView className="flex-1 bg-white">
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled">
        <View className="flex-1 px-6 pt-4">
          ...
          <Text className="text-2xl font-extrabold text-slate-900 mb-2">Criar conta profissional</Text>
          <Text className="text-slate-500 mb-8">Gratuito. Sem cartão de crédito.</Text>

          {step === 'info' && (
            <View className="gap-4">
              <Input label="Seu nome" placeholder="Ex: João Silva" value={name} onChangeText={setName} />
              <Input
                label="Telefone (com DDD)" placeholder="11999990000"
